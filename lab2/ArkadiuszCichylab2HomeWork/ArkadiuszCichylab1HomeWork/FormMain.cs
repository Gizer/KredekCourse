...
using ArkadiuszCichyLab2Homework;
...
namespace ArkadiuszCichylab1HomeWork
{
    public partial class FormMain : Form
    {
        ...
        public static int deficiency { get; set; }
        ...
        public static System.Media.SoundPlayer player = new System.Media.SoundPlayer("Resources/sound/HoMM3main.wav");

        // List of Units objects
        public static List<Unit> playerUnits = new List<Unit>();

        // When archer level up enough, player should get MasterArcher so this value changes to true
        public static bool getMasterArcher = false;

        // Window for fights
        private FormFight formFight;
        ...
        public FormMain()
        {
            ...
            // Adding Units to buy
            comboBoxBuyUnits.Items.Add(new Archer());
            comboBoxBuyUnits.Items.Add(new Warrior());
            comboBoxBuyUnits.Items.Add(new Mage());
        ...
        /// <summary>
        /// Setting units after fight
        /// </summary>
        /// <param name="units">list of units after fight</param>
        public static void SetPlayerUnits(ref List<Unit> units)
        {
            playerUnits = units;
        }

        /// <summary>
        /// Passing player units to fight window
        /// </summary>
        /// <returns></returns>
        public static ref List<Unit> GetPlayerUnits()
        {
            return ref playerUnits;
        }
        ...
        {
            labelWoodNumber.Text = wood.ToString() + $" ({woodIncome.ToString()})";
            labelStoneNumber.Text = stone.ToString() + $" ({stoneIncome.ToString()})";
            labelGemsNumber.Text = gems.ToString() + $" ({gemsIncome.ToString()})";
            labelFoodNumber.Text = food.ToString() + $" ({foodIncome.ToString()})";
            labelMoneyNumber.Text = money.ToString() + $" ({moneyIncome.ToString()})";
            labelDeficiencyNumber.Text = deficiency.ToString();
            labelPassedExams.Text = "Zdane egzaminy:\r\n" + winCount.ToString();
        }
        ...
        /// <summary>
        /// Buying unit procedure
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void buttonBuy_Click(object sender, EventArgs e)
        {
            if (money >= 5000 && comboBoxBuyUnits.SelectedIndex != -1 && !playerUnits.Contains((Unit)comboBoxBuyUnits.SelectedItem))
            {
                money -= 5000;
                playerUnits.Add((Unit)comboBoxBuyUnits.SelectedItem);
                UpdatePlayerUnitsLabel();
            }
            else
                MessageBox.Show("Nie stać cię na kupno jednostki lub już ją masz");
        }

        /// <summary>
        /// Updating playerUnits label
        /// </summary>
        private void UpdatePlayerUnitsLabel()
        {
            labelPlayerUnits.Text = "";
            foreach (Unit unit in playerUnits)
                labelPlayerUnits.Text += unit.ToString() + "\r\n";
        }

        /// <summary>
        /// Start fight procedure
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void buttonFight_Click(object sender, EventArgs e)
        {
            // You can fight only when you have units
            if (playerUnits.Count > 0)
            {
                timerEvilEvents.Stop();
                timerResources.Stop();
                formFight = new FormFight(this);
                formFight.ShowDialog(); // Fight is in dialog window so FormMain cannot be accesed
                if (winCount > 10)
                {
                    MessageBox.Show("Twoi poddani dokonali niemożliwego!\r\nZaliczyli sesję i zdali studia\r\n\r\nWygrałeś!");
                    this.Close();
                }
                if (getMasterArcher == true)
                {
                    foreach (Unit unit in playerUnits)
                        if (unit.ToString() == "Archer")
                        {
                            playerUnits.Remove(unit);
                            break;
                        }
                    playerUnits.Add(new MasterArcher());
                    getMasterArcher = false;
                }
                timerEvilEvents.Start();
                UpdatePlayerUnitsLabel();
                timerResources.Start();
            ...
            {
                MessageBox.Show("Nie masz jednostek aby walczyć o 3.0");
            }
        }
    }
}